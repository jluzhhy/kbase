package unlzhhy__ContigFilter::unlzhhy__ContigFilterClient;
...
unlzhhy__ContigFilter::unlzhhy__ContigFilterClient

=head1 DESCRIPTION


A KBase module: unlzhhy__ContigFilter
...
sub new
{
    ...
    my $self = {
	client => unlzhhy__ContigFilter::unlzhhy__ContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is an unlzhhy__ContigFilter.FilterContigsParams
$return is an unlzhhy__ContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is an unlzhhy__ContigFilter.workspace_name
	contigset_id has a value which is an unlzhhy__ContigFilter.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is an unlzhhy__ContigFilter.ws_contigset_id
...
# Authentication: required
	...
	method => "unlzhhy__ContigFilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$params is an unlzhhy__ContigFilter.FilterContigsMaxParams
$return is an unlzhhy__ContigFilter.FilterContigsResults
...
# Authentication: required
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
	method => "unlzhhy__ContigFilter.filter_contigs_max",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "unlzhhy__ContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for unlzhhy__ContigFilter::unlzhhy__ContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "unlzhhy__ContigFilter::unlzhhy__ContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package unlzhhy__ContigFilter::unlzhhy__ContigFilterClient::RpcClient;
