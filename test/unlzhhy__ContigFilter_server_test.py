...
class unlzhhy__ContigFilterTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'unlzhhy__ContigFilter',
        ...
        for nameval in config.items('unlzhhy__ContigFilter'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = unlzhhy__ContigFilter(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_unlzhhy__ContigFilter_" + str(suffix)
